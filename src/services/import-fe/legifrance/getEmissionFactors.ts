...
import {
  LegifranceEmissionFactor,
  beRequiredColumns,
  mapLegifranceEmissionFactors,
...
const numberColumns: (keyof LegifranceEmissionFactor)[] = [
...
export const getEmissionFactorsFromCSV = async (name: string, file: string, reseau: 'froid' | 'chaud') => {
  await prismaClient.$transaction(
    async (transaction) => {
      ...
      const emissionFactors: LegifranceEmissionFactor[] = []
      const parts: LegifranceEmissionFactor[] = []
      ...
      await new Promise<void>((resolve, reject) => {
        fs.createReadStream(file)
          .pipe(
            parse({
              ...
              cast: (value, context) => {
                ...
                if (numberColumns.includes(context.column as keyof LegifranceEmissionFactor)) {
          ...
          .on('data', (row: LegifranceEmissionFactor) => {
          ...
          .on('end', async () => {
            ...
            for (const emissionFactor of emissionFactors) {
              ...
              const data = mapLegifranceEmissionFactors(emissionFactor, emissionFactorImportVersion.id, reseau)
