...
jest.mock('./user', () => ({
  ...
  canEditSelfRole: jest.fn(),
  canDeleteMember: jest.fn(),
}))
...
const mockCanAddMember = canAddMember as jest.Mock
const mockCanEditSelfRole = canEditSelfRole as unknown as jest.Mock
const mockCanDeleteMember = canDeleteMember as jest.Mock
const mockCanChangeRole = canChangeRole as jest.Mock
...
describe('User permission functions', () => {
  ...
  describe('canEditSelfRole', () => {
    it('returns false is role is Super Admin', () => {
      expect(mockCanEditSelfRole(Role.SUPER_ADMIN)).toBe(false)
    })

    it('returns false is role is Collaborator', () => {
      expect(mockCanEditSelfRole(Role.COLLABORATOR)).toBe(false)
    })

    it('returns false is role is Member', () => {
      expect(mockCanEditSelfRole(Role.DEFAULT)).toBe(false)
    })

    it('returns true is role is Admin', () => {
      expect(mockCanEditSelfRole(Role.ADMIN)).toBe(true)
    })

    it('returns true is role is Gestionnaire', () => {
      expect(mockCanEditSelfRole(Role.GESTIONNAIRE)).toBe(true)
    })
  })

  describe('canAddMember', () => {
    it('returns false if organizationVersionId is null', () => {
      const result = mockCanAddMember(adminUser, { role: Role.ADMIN }, null)
    ...
    it('returns false if trying to add SUPER_ADMIN', () => {
      mockCanEditMemberRole.mockReturnValue(true)
      const result = mockCanAddMember(adminUser, { role: Role.SUPER_ADMIN }, mockedOrganizationVersionId)
      expect(result).toBe(false)
      expect(mockCanEditMemberRole).toHaveBeenCalledTimes(1)
    })
    ...
    it('returns true when has rights, target is not SUPER_ADMIN and organization matches', () => {
      mockCanEditMemberRole.mockReturnValue(true)
      const result = mockCanAddMember(adminUser, { role: Role.GESTIONNAIRE }, mockedOrganizationVersionId)
      expect(result).toBe(true)
      expect(mockCanEditMemberRole).toHaveBeenCalledTimes(1)
    })
  })
  ...
  describe('canDeleteMember', () => {
    it('returns false if member is null', () => {
      expect(mockCanDeleteMember(adminUser, null)).toBe(false)
      expect(mockCanEditMemberRole).toHaveBeenCalledTimes(0)
    })

    it('returns false if member is not from user organization', () => {
      const result = mockCanDeleteMember(
  ...
  describe('mockCanChangeRole', () => {
    it('returns false if member is null', () => {
      const result = mockCanChangeRole(adminUser, null, Role.ADMIN)
      expect(result).toBe(false)
      expect(mockCanEditMemberRole).toHaveBeenCalledTimes(0)
    ...
    it('returns false if editing own role without proper rights', () => {
      const collaboratorResult = mockCanChangeRole(
      ...
      const memberResult = mockCanChangeRole(
    ...
    it('returns false if cannot edit member roles', () => {
      mockCanEditMemberRole.mockReturnValue(false)
      const result = mockCanChangeRole(adminUser, getMockedDbAccount() as AccountWithUser, Role.ADMIN)
      expect(result).toBe(false)
      expect(mockCanEditMemberRole).toHaveBeenCalledTimes(1)
      expect(mockCanBeUntrainedRole).toHaveBeenCalledTimes(0)
    })
    ...
    it('returns false if user is from another organization', () => {
      mockCanEditMemberRole.mockReturnValue(true)
      const result = mockCanChangeRole(
        adminUser,
    ...
    it('returns false if assigning SUPER_ADMIN', () => {
      mockCanEditMemberRole.mockReturnValue(true)
      const result = mockCanChangeRole(adminUser, getMockedDbAccount() as AccountWithUser, Role.SUPER_ADMIN)
      expect(result).toBe(false)
      expect(mockCanEditMemberRole).toHaveBeenCalledTimes(1)
      expect(mockCanBeUntrainedRole).toHaveBeenCalledTimes(0)
    })
    ...
    it('returns true when all checks pass', () => {
      ...
      const untrainedResult = mockCanChangeRole(
        adminUser,
        getMockedDbAccount({}, { level: null }) as AccountWithUser,
        Role.GESTIONNAIRE,
      )
      ...
      mockCanEditMemberRole.mockReturnValue(true)
      mockCanBeUntrainedRole.mockReturnValue(false)
      const trainedResult = mockCanChangeRole(adminUser, getMockedDbAccount() as AccountWithUser, Role.ADMIN)
