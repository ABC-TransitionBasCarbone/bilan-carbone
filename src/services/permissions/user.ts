...
export const findUserInfo = (user: Account) =>
  ({
    select: {
      user: {
        select: {
...
export const canDeleteMember = (user: Account, member: DbUser | null) => {
  if (!member) {
    return false
  }
...
export const canChangeRole = (user: Account, member: AccountWithUser | null, newRole: Role) => {
  if (!member) {
    return false
  }
