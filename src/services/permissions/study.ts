...
export const isAdminOnStudyOrga = (user: Account, studyOrganization: Pick<Organization, 'id' | 'parentId'>) =>
  isAdminOnOrga(user, studyOrganization)

export const canReadStudy = async (user: Account | UserWithAllowedStudies, studyId: string) => {
  if (!user) {
    return false
  }
...
export const filterAllowedStudies = async (user: Account, studies: Study[]) => {
...
const canChangeStudyValues = async (user: Account, study: FullStudy) => {
...
export const canChangePublicStatus = async (user: Account, study: FullStudy) => {
  return canChangeStudyValues(user, study)
}

export const canUpgradeSourceVersion = async (user: Account, study: FullStudy) => {
  return canChangeStudyValues(user, study)
}

export const canChangeDates = async (user: Account, study: FullStudy) => {
  return canChangeStudyValues(user, study)
}

export const canChangeSites = async (user: Account, study: FullStudy) => {
  return canChangeStudyValues(user, study)
}

export const canChangeLevel = async (user: Account, study: FullStudy, level: Level) => {
...
export const canChangeResultsUnit = async (user: Account, study: FullStudy) => {
  return canChangeStudyValues(user, study)
}

export const canChangeName = async (user: Account, study: FullStudy) => {
  return canChangeStudyValues(user, study)
}

export const canChangeOpeningHours = async (user: Account, study: FullStudy) => {
  return canChangeStudyValues(user, study)
}
...
export const canAddContributorOnStudy = (user: Account, study: FullStudy) => {
  if (isAdminOnStudyOrga(user, study.organization)) {
    return true
  }
...
export const canDeleteStudy = async (studyId: string) => {
  ...
  if (
    study.allowedUsers.some(
      (allowedUser) =>
        allowedUser.role === StudyRole.Validator && allowedUser.account.user.email === session.user.email,
    )
  ) {
    return true
  }
...
export const filterStudyDetail = (user: Account, study: FullStudy) => {
...
export const canReadStudyDetail = async (user: Account, study: FullStudy) => {
