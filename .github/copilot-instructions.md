...
- **MIP Results Dashboard**:
  - Campaign results live in `apps/mip/src/app/survey/[id]/results/page.tsx` and `apps/mip/src/components/results/ResultsDashboard.tsx`, with sample data in `apps/mip/src/data/sampleResults.ts`.
  - For chart rendering, align with `apps/bilan-carbone/src/app/(public)/preview/etudes/[id]/ChartsPage.tsx`: include a donut/pie chart for average emissions split by category and a horizontal bar chart for footprint distribution.
  - The horizontal bar chart must show the 2050 objective marker at **2 tCO₂e** to compare each category or entity footprint against the target.
