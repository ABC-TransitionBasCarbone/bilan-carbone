...
import BaseStyledChip from './BaseStyledChip'
import styles from './StyledChip.module.css'

type PolymorphicChipProps<C extends ElementType = 'div'> = ChipProps<C> & {
    component?: C
    subtitle?: string
    roleClass?: string
}

const StyledChip = <C extends ElementType = 'div'>({
    label,
    subtitle,
    roleClass,
    className,
    ...
    const chip = (
        <BaseStyledChip
            ...
            label={finalLabel}
            {...props}
            className={classNames(roleClass && styles[roleClass], className)}
        />
    )
